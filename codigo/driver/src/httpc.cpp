...
static size_t header_callback(const char *buffer, size_t size, size_t nitems, void *userdata)
{
    ...
    auto value{
        std::string(buffer + separator_idx + 2, line.size() - separator_idx - 4)}; // skipping ": " and "\r\n"
    // TODO(mredolatti): LOG!
...
Client::response_t Client::execute(const Request& request)
{
    ...
    if (request.tls_conf()) {
        ...
        curl_easy_setopt(*handle, CURLOPT_CAINFO,
                         cfg.root_ca_fn().empty() ? nullptr : cfg.root_ca_fn().c_str());
        curl_easy_setopt(*handle, CURLOPT_SSLKEY,
                         cfg.client_pk_fn().empty() ? nullptr : cfg.client_pk_fn().c_str());
        curl_easy_setopt(*handle, CURLOPT_SSLCERT,
                         cfg.client_cert_fn().empty() ? nullptr : cfg.client_cert_fn().c_str());
    }
    ...
    case Method::DELETE: curl_easy_setopt(*handle, CURLOPT_CUSTOMREQUEST, "DELETE"); break;
