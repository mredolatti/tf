...
CacheEntry::CacheEntry(std::string file_id, std::string file_name, std::string contents,
                       sync_time_t last_sync)
...
const std::string& CacheEntry::file_id() const { return file_id_; }

const std::string& CacheEntry::file_name() const { return file_name_; }

std::string& CacheEntry::contents() { return contents_; }

std::chrono::system_clock::time_point CacheEntry::last_sync() const { return last_sync_; }

bool CacheEntry::dirty() const { return dirty_; }
...
FileCache::get_res_t FileCache::get(const std::string& org_name, const std::string& server_id,
...
bool FileCache::put(const std::string& org_name, std::string server_id, std::string ref, std::string contents)
{
    ...
    return entries_
        .insert({key, detail::CacheEntry{std::move(server_id), key, std::move(contents),
