...
FSMirror::Error FSMirror::mkdir(path_t path)
{
    ...
    return (root_.insert(path, std::make_unique<fstree::InnerNode>(path.filename()))) ? Error::Ok
                                                                                      : Error::AlreadyExists;
}
...
FSMirror::Error FSMirror::reset_all(const std::vector<models::Mapping>& mappings)
{
    ...
    for (const auto& mapping : mappings) {
        files_ok &= (add_file(mapping.org(), mapping.server(), mapping.ref(), mapping.size_bytes(),
                              mapping.last_updated()) == Error::Ok);
        if (!mapping.path().empty()) {
            mappings_ok &= (link_file(mapping.id(), mapping.org(), mapping.server(), mapping.ref(),
                                      mapping.path()) == Error::Ok);
        }
    }
