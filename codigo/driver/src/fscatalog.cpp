...
ServerInfo::ServerInfo(std::string_view organization, std::string_view server, std::string_view files_url,
...
const std::string& ServerInfo::organization() const { return organization_; }

const std::string& ServerInfo::server() const { return server_; }

const std::string& ServerInfo::files_url() const { return files_url_; }

const tls::Config& ServerInfo::tls_config() const { return tls_config_; }

void ServerInfo::update_fetch_url(std::string_view new_url) { files_url_ = new_url; }
...
FileServerCatalog::ptr_t
FileServerCatalog::createFromCredentialsConfig(const mifs::Config::fs_creds_by_org_t& creds_by_org)
{
    catalog_t catalog;
    for (const auto& for_org : creds_by_org) {
        for (const auto& for_server : for_org.second) {
            catalog.insert({make_catalog_key(for_org.first, for_server.first),
                            ServerInfo{for_org.first, for_server.first, "",
...
bool FileServerCatalog::update_fetch_url(std::string_view org, std::string_view server,
